...
- **Donation placeholder:** the literal string `BMC_USERNAME_PLACEHOLDER` must be used for the Buy Me a Coffee username in exactly two files (`README.md`, `.github/FUNDING.yml`) so one find-and-replace swaps it (spec §5).
...
**Interfaces:**
- Consumes: `docs/brand/ravenspire-banner.svg` from Task 2; `BMC_USERNAME_PLACEHOLDER` contract from Global Constraints.
...
<p align="center">
  ...
  <a href="https://buymeacoffee.com/BMC_USERNAME_PLACEHOLDER"><img src="https://img.shields.io/badge/%F0%9F%8D%BA_buy_me_a_beer-ffbe4a" alt="Buy me a beer"></a>
</p>
...
Run: `node -e "const fs=require('fs');const md=fs.readFileSync('README.md','utf8');for(const f of ['docs/brand/ravenspire-banner.svg','ravenspire-rpg.png','ravenspire-dashboard.png','ravenspire-history.png']){if(!md.includes(f))throw new Error('README missing ref: '+f);if(!fs.existsSync(f))throw new Error('file missing: '+f)};if((md.match(/BMC_USERNAME_PLACEHOLDER/g)||[]).length!==1)throw new Error('expected exactly 1 placeholder in README header task');console.log('README refs ok')"`
...
**Interfaces:**
- Consumes: `BMC_USERNAME_PLACEHOLDER` contract. After this task the placeholder appears exactly **twice in README.md** (badge + support button) **and once in FUNDING.yml**.
...
<a href="https://buymeacoffee.com/BMC_USERNAME_PLACEHOLDER"><img src="https://img.shields.io/badge/%F0%9F%8D%BA_Buy_me_a_beer-ffbe4a?style=for-the-badge" alt="Buy me a beer"></a>
...
```yaml
# GitHub "Sponsor" button → Buy Me a Coffee.
# LAUNCH CHECKLIST: replace BMC_USERNAME_PLACEHOLDER with the real username
# (also twice in README.md) before any public announcement.
buy_me_a_coffee: BMC_USERNAME_PLACEHOLDER
```

- [ ] **Step 3: Verify placeholder count and YAML shape**

Run: `node -e "const fs=require('fs');const md=fs.readFileSync('README.md','utf8');const fy=fs.readFileSync('.github/FUNDING.yml','utf8');const n=(md.match(/BMC_USERNAME_PLACEHOLDER/g)||[]).length;if(n!==2)throw new Error('README placeholders: '+n+' (want 2)');if(!/^buy_me_a_coffee: BMC_USERNAME_PLACEHOLDER$/m.test(fy))throw new Error('FUNDING.yml malformed');console.log('donations wiring ok')"`
...
1. Create the Buy Me a Coffee account (~2 min, buymeacoffee.com) → replace `BMC_USERNAME_PLACEHOLDER` in `README.md` (×2) and `.github/FUNDING.yml` (×1).
